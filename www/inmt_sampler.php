...
if (flock($fp, LOCK_EX))
{
	...
	$url = '158.42.161.14:6542/?source='.urlencode($source).'&prefix='.urlencode($prefix).'&excluded='.urlencode($excluded).'&learn='.urlencode($learn).'&beam_size='.urlencode($beam_size).'&length_norm='.urlencode($length_norm).'&coverage_norm='.urlencode($coverage_norm).'&alpha_norm='.urlencode($alpha_norm);
    $ch = curl_init();
